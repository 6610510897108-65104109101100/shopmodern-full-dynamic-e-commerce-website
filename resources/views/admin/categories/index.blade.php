...
@section('title', 'Collections — ShopModern')
...
<div class="mb-10">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center gap-3 px-8 py-4 bg-primary text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all">
    ...
    @foreach($categories as $cat)
        <div class="group bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 flex flex-col items-center text-center transition-all hover:shadow-2xl hover:shadow-slate-200/50 dark:hover:shadow-none hover:-translate-y-2">
            <div class="relative w-full aspect-[4/5] rounded-[2rem] overflow-hidden bg-slate-50 dark:bg-slate-800 mb-8 shadow-inner">
                <img src="{{ $cat->image_url }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="{{ $cat->name }}">
                <div class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            </div>

            <div class="flex-grow w-full">
                <h3 class="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-2">{{ $cat->name }}</h3>
                <p class="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-4">{{ $cat->products_count }} PIECES</p>
            ...
            <div class="w-full flex items-center justify-center gap-3 border-t border-slate-50 dark:border-slate-800/50 pt-6">
                <a href="{{ route('admin.categories.edit', $cat) }}" class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-primary hover:bg-primary/5 transition-all">
                    <span class="material-symbols-outlined text-lg">edit</span>
                </a>
                <form action="{{ route('admin.categories.destroy', $cat) }}" method="POST" onsubmit="return confirm('Archive this entire collection?')">
                    @csrf @method('DELETE')
