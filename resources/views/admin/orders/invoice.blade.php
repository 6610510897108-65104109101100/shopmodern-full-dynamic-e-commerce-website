...
<body class="bg-slate-50 p-4 md:p-10">
    ...
    <div class="max-w-4xl mx-auto no-print mb-8 flex justify-between items-center">
        <a href="{{ route('admin.orders.index') }}" class="text-slate-500 font-bold hover:text-slate-900 flex items-center gap-2">
            ← Back to Orders
        </a>
        ...
        <div class="p-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
                <div>
                    ...
                    <div class="text-xl font-black text-slate-900 mb-1">{{ $order->customer_name }}</div>
                    <div class="text-slate-500 font-medium mb-1">{{ $order->customer_email }}</div>
                    @if($order->customer_phone)
                        <div class="text-slate-500 font-medium mb-4">{{ $order->customer_phone }}</div>
                    @endif
                    <div class="text-sm text-slate-400 font-bold uppercase tracking-tight">Shipping Address:</div>
                    <div class="text-slate-600 font-medium leading-relaxed">{{ $order->shipping_address ?? 'Standard Shipping' }}</div>
                </div>
                <div class="md:text-right">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between md:justify-end gap-8">
                            ...
                            <span class="text-slate-900 font-black text-sm">{{ $order->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="flex justify-between md:justify-end gap-8">
                            <span class="text-slate-400 font-bold text-sm">Payment State:</span>
                            <span class="text-emerald-600 font-black text-sm uppercase tracking-widest">{{ $order->payment_status }}</span>
                        </div>
                        <div class="flex justify-between md:justify-end gap-8">
                            <span class="text-slate-400 font-bold text-sm">Delivery:</span>
                            <span class="text-primary font-black text-sm uppercase tracking-widest">{{ $order->status }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($order->items as $item)
                        <tr>
                            ...
                            <td class="py-6 text-center font-bold text-slate-600">x{{ $item->quantity }}</td>
                            <td class="py-6 text-right font-bold text-slate-600">${{ number_format($item->price_cents / 100, 2) }}</td>
                            <td class="py-6 text-right font-black text-slate-900">${{ number_format(($item->price_cents * $item->quantity) / 100, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div class="flex justify-end pt-12 border-t-2 border-slate-100">
                <div class="w-full max-w-xs space-y-4">
                    <div class="flex justify-between">
                        ...
                        <span class="text-slate-900 font-bold">${{ number_format($order->subtotal_cents / 100, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-400 font-bold uppercase tracking-widest text-xs">Shipping</span>
                        <span class="text-slate-900 font-bold">${{ number_format($order->shipping_cents / 100, 2) }}</span>
                    </div>
                    <div class="flex justify-between pt-4 border-t border-slate-100">
                        <span class="text-slate-900 font-black uppercase tracking-widest">Total Pay</span>
                        <span class="text-3xl font-black text-primary">${{ $order->total }}</span>
                    </div>
                </div>
            </div>
