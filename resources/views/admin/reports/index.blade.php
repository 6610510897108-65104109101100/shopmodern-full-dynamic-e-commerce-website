...
@section('title', 'Sales & Revenue Reports')
...
<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
    ...
    <form method="GET" action="{{ route('admin.reports.index') }}" class="flex flex-wrap items-center gap-3">
        <div class="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl ring-1 ring-slate-200 dark:ring-slate-800 shadow-sm">
            ...
            <input type="date" name="start_date" value="{{ $startDate }}" class="bg-transparent border-none text-xs font-bold focus:ring-0 p-0 w-32">
            <span class="text-slate-300 font-bold">to</span>
            <input type="date" name="end_date" value="{{ $endDate }}" class="bg-transparent border-none text-xs font-bold focus:ring-0 p-0 w-32">
        </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
    <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/40 relative overflow-hidden group">
        ...
        <div class="text-4xl font-black text-slate-900 dark:text-white">${{ number_format($summary->total_revenue / 100, 2) }}</div>
    ...
    <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/40 relative overflow-hidden group">
        ...
        <div class="text-4xl font-black text-slate-900 dark:text-white">{{ $summary->total_orders }}</div>
    ...
    <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/40 relative overflow-hidden group">
        ...
        <div class="text-4xl font-black text-slate-900 dark:text-white">${{ number_format($summary->avg_order_value / 100, 2) }}</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col">
            ...
            @foreach($recentPayments as $p)
                <div class="flex items-center justify-between group cursor-pointer">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-sm font-black text-slate-900 dark:text-white">{{ $p->order_number }}</div>
                            <div class="text-[10px] font-bold text-slate-400 capitalize">{{ $p->created_at->diffForHumans() }}</div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm font-black text-emerald-600">+${{ $order->total }}</div>
                        <div class="text-[9px] text-slate-400 font-bold uppercase">{{ $p->customer_name }}</div>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-b-3xl">
            <a href="{{ route('admin.orders.index', ['status' => 'paid']) }}" class="w-full flex items-center justify-center gap-2 text-xs font-black text-primary uppercase tracking-widest hover:underline transition-all">
...
<script>
    ...
    const chartData = {!! json_encode($dailyRevenue) !!};
