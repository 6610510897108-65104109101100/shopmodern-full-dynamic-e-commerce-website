...
@section('title', 'ShopModern — Defined Luxury')

@section('content')
    {{-- Hero Section --}}
    <section class="relative h-[80vh] w-full mb-20 overflow-hidden rounded-[3rem] shadow-2xl">
        <img src="{{ isset($settings['hero_image']) ? asset($settings['hero_image']) : 'file:///C:/Users/User/.gemini/antigravity/brain/77e9f5b9-ccdc-48f8-9fc5-29b51239903a/fashion_hero_banner_1775395815592.png' }}" 
             class="absolute inset-0 w-full h-full object-cover transition-transform duration-[20s] hover:scale-110" alt="{{ $settings['hero_title'] ?? 'New Season' }}">
        <div class="absolute inset-0 bg-black/30"></div>
        <div class="relative h-full flex flex-col justify-center items-center text-center px-6">
            <h1 class="font-display text-7xl md:text-9xl font-black text-white tracking-tighter mb-4 uppercase mix-blend-overlay">{{ $settings['hero_title'] ?? 'NEW SEASON' }}</h1>
            <p class="text-white/90 text-lg md:text-xl font-medium tracking-[0.3em] uppercase mb-10">{{ $settings['hero_subtitle'] ?? 'Autumn / Winter Collectio 2026' }}</p>
            <a href="{{ $settings['hero_button_link'] ?? route('shop') }}" class="group relative px-12 py-5 bg-white text-black font-display font-black uppercase tracking-widest overflow-hidden transition-all rounded-full hover:px-16">
                <span class="relative z-10">{{ $settings['hero_button_text'] ?? 'Shop Collection' }}</span>
    ...
    <section class="mb-32">
        <div class="flex items-end justify-between mb-12">
            ...
            <a href="{{ route('shop') }}" class="text-xs font-black uppercase tracking-tighter border-b-2 border-primary pb-1">View Archive</a>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach($categories as $cat)
                <a href="{{ route('shop', ['category' => $cat->slug]) }}" class="group block relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-slate-100">
                    <img class="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" 
                         src="{{ $cat->image_url ?? 'https://placehold.co/800x1000?text=' . $cat->name }}" 
                         alt="{{ $cat->name }}">
                    ...
                    <div class="absolute bottom-10 left-10 text-white">
                        <div class="text-2xl font-display font-black tracking-tighter">{{ $cat->name }}</div>
                ...
                <div class="group h-full flex flex-col relative">
                    <div class="relative aspect-[3/4] overflow-hidden rounded-[1.5rem] bg-slate-50 mb-6">
                        <a href="{{ route('products.show', $p->slug) }}" class="block h-full">
                            <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" 
                                 src="{{ $p->images->first()->url ?? 'https://placehold.co/800x1200' }}" 
                                 alt="{{ $p->name }}">
                        </a>
                    ...
                    <a href="{{ route('products.show', $p->slug) }}" class="px-2 block flex-grow">
                        <div class="flex justify-between items-start mb-2">
                            <h4 class="text-base font-black text-slate-900 group-hover:text-amber-600 transition-colors leading-tight truncate mr-2">{{ $p->name }}</h4>
                            <span class="font-display font-bold text-lg leading-none">৳{{ $p->price }}</span>
                        </div>
                        <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            ...
                            @if($p->compare_at_price)
                                <span class="line-through opacity-50">৳{{ $p->compare_at_price }}</span>
                            @endif
                        </div>
                    </a>
                </div>
