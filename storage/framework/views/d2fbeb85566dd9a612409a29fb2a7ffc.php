...
<?php $__env->startSection('title', 'Shopping Vault — ShopModern'); ?>

<?php $__env->startSection('content'); ?>
<div class="mb-16">
    <h1 class="font-display text-5xl font-black tracking-tighter uppercase italic">Shopping Vault</h1>
    <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2">Inventory Reserved for You (<?php echo e($items->count()); ?> Pieces)</p>
</div>
...
<?php if($items->isEmpty()): ?>
    <div class="py-32 text-center rounded-[3rem] bg-slate-50 border-2 border-dashed border-slate-200">
        ...
        <a href="<?php echo e(route('shop')); ?>" class="mt-8 inline-block px-10 py-4 bg-black text-white font-black uppercase tracking-widest text-xs rounded-2xl hover:scale-105 transition-all">Go Shop</a>
    </div>
<?php else: ?>
                    ...
                    <div class="flex flex-col sm:flex-row gap-8 pb-10 border-b border-slate-100 group">
                        <div class="w-full sm:w-40 aspect-[3/4] rounded-3xl overflow-hidden bg-slate-50 shadow-sm">
                            <img class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" 
                                 src="<?php echo e($p->images->first()->url ?? 'https://placehold.co/400x600'); ?>" alt="<?php echo e($p->name); ?>">
                        </div>
                        <div class="flex-1 flex flex-col justify-between py-2">
                            <div>
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 class="text-2xl font-display font-black text-black tracking-tighter uppercase leading-none mb-2"><?php echo e($p->name); ?></h3>
                                        <div class="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                            <span>Collection: <?php echo e($p->category->name); ?></span>
                                            <?php if($item->size): ?> <span>Size: <?php echo e($item->size); ?></span> <?php endif; ?>
                                            <?php if($item->color): ?> <span>Color: <?php echo e($item->color); ?></span> <?php endif; ?>
                                        </div>
                                    </div>
                                    <span class="text-2xl font-display font-black text-black leading-none">$<?php echo e($p->price); ?></span>
                                </div>
                            </div>
                            
                            <div class="flex items-center justify-between mt-8">
                                <div class="flex items-center gap-4">
                                    <form method="POST" action="<?php echo e(route('cart.update', $item)); ?>" class="flex items-center bg-slate-50 rounded-2xl p-1 border border-slate-100">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('PATCH'); ?>
                                        <button name="quantity" value="<?php echo e(max(1, $item->quantity - 1)); ?>" class="w-10 h-10 flex items-center justify-center font-black text-lg hover:text-accent transition-colors disabled:opacity-20" <?php if($item->quantity <= 1): echo 'disabled'; endif; ?>>-</button>
                                        <span class="w-10 text-center font-display font-black text-sm"><?php echo e($item->quantity); ?></span>
                                        <button name="quantity" value="<?php echo e($item->quantity + 1); ?>" class="w-10 h-10 flex items-center justify-center font-black text-lg hover:text-accent transition-colors">+</button>
                                    </form>
                                ...
                                <div class="text-right">
                                    ...
                                    <span class="text-xl font-display font-black text-black leading-none">$<?php echo e(number_format(($p->price_cents * $item->quantity)/100, 2)); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-black text-white rounded-[3rem] p-10 shadow-2xl">
                ...
                <div class="space-y-6">
                    <div class="flex justify-between items-end">
                        ...
                        <span class="font-display font-black text-2xl">$<?php echo e(number_format($totals['subtotalCents']/100, 2)); ?></span>
                    </div>
                    ...
                    <div class="pt-10 border-t border-white/10 mt-10">
                        <div class="flex justify-between items-end mb-10">
                            ...
                            <span class="font-display font-black text-5xl leading-none italic underline decoration-accent underline-offset-4">$<?php echo e(number_format($totals['totalCents']/100, 2)); ?></span>
                        </div>

                        <a href="<?php echo e(route('checkout')); ?>" class="group w-full block py-6 bg-accent text-white font-display font-black text-sm uppercase tracking-[0.2em] rounded-3xl hover:bg-white hover:text-black transition-all text-center shadow-xl shadow-accent/20">
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp3\htdocs\ShopModern\shopmodern\shopmodern\resources\views/public/cart.blade.php ENDPATH**/ ?>